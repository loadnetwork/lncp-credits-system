...
if not PaymentTokenUsdPrice then PaymentTokenUsdPrice = 0 end 
if not PaymentTokenTicker then PaymentTokenTicker = "AO" end
-- 18 for dumdum
if not PaymentTokenDenomination then PaymentTokenDenomination = 18 end
if not Credits then Credits = {} end

-- Admin functions --

-- 1. update payment token USD price
Handlers.add("UpdatePaymentTokenPrice", Handlers.utils.hasMatchingTag("Action", "UpdatePaymentTokenPrice"), function(msg)
...
Handlers.add("WithdrawPaymentsTokens",
...
Handlers.add("BatchReduceCredits",
...
Handlers.add('Balances', "Balances",
  function(msg) msg.reply({ Data = json.encode(Credits) }) end)
...
Handlers.add("BuyCredits",
    ...
    function(msg)
        ...
        if not Credits[sender] then Credits[sender] = 0 end
